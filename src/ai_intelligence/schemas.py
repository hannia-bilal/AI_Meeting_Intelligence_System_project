...
class MeetingAnalysisRequest(BaseModel):
    ...
    speaker_name_overrides: Optional[Dict[str, str]] = Field(
        default=None,
        description="Optional manual mapping from speaker IDs to user-assigned names (e.g., {'SPEAKER_00': 'Sarah', 'SPEAKER_01': 'Ali'}) as per project discussion"
    )
...
class EnrichedTranscriptSegment(BaseModel):
    """
    Enriched transcript segment fulfilling 'Speaker-wise transcript' deliverable.
    Combines speaker ID, display name, timestamps, and transcribed text.
    """
    speaker_id: str = Field(..., description="Original speaker label, e.g. 'SPEAKER_00'")
    speaker_name: str = Field(..., description="Display speaker name (e.g. 'Ali' or 'SPEAKER_00')")
    start: float = Field(..., description="Start timestamp in seconds", ge=0.0)
    end: float = Field(..., description="End timestamp in seconds", ge=0.0)
    timestamp_formatted: str = Field(..., description="Formatted timestamp (e.g. '02:15')")
    text: str = Field(..., description="Spoken transcript text")


class DashboardMetrics(BaseModel):
    """
    Key metrics aggregated specifically for Dashboard view requirements.
    """
    duration_formatted: str = Field(..., description="Human readable duration string (e.g. '30m 45s' or '01:02:15')")
    action_items_count: int = Field(default=0, description="Total number of extracted action items")
    decisions_count: int = Field(default=0, description="Total number of decisions made")
    unresolved_issues_count: int = Field(default=0, description="Count of open questions or blockers")
    upcoming_deadlines_count: int = Field(default=0, description="Count of impending deadlines")
    overall_sentiment_label: str = Field(default="neutral", description="Positive, Neutral, Negative, Mixed")
...
class MeetingIntelligenceReport(BaseModel):
    ...
    speaker_wise_transcript: List[EnrichedTranscriptSegment] = Field(
        default_factory=list,
        description="Enriched speaker-wise transcript with formatted timestamps and resolved names"
    )
    ...
    dashboard_metrics: Optional[DashboardMetrics] = Field(
        default=None,
        description="Pre-calculated dashboard summary metrics"
    )
...
# ==============================================================================
# Meeting Q&A Schemas ("Ask AI" feature with timestamp references)
# ==============================================================================

class MeetingQARequest(BaseModel):
    """
    Question payload for Ask AI feature.
    """
    question: str = Field(
        ...,
        description="User question about the meeting (e.g. 'What did we decide about the marketing budget?')",
        examples=["What did we decide about the marketing budget?"]
    )
    transcript_text: Optional[str] = Field(
        default=None,
        description="Optional pre-formatted transcript text"
    )
    meeting_title: Optional[str] = Field(default=None)


class MeetingQAAnswer(BaseModel):
    """
    Answer payload providing answer, relevant timestamps, and referenced speakers.
    """
    question: str
    answer: str
    relevant_timestamps: List[TimestampReference] = Field(
        default_factory=list,
        description="Timestamp references matching document requirement (e.g. '32:45')"
    )
    referenced_speakers: List[str] = Field(default_factory=list)
